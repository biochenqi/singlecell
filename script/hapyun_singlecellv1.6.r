...
if (isstep(3)){
  ...
  if(cscumap=="umap"){
    ...
    pbmcall=RunUMAP(pbmcall,dims = 1:csc$step3$dims)
...
if (isstep(4)){
  ...
  if(csc$step4$clustermarkers){
    ...
    write.table(subset(pbmcall.markers,avg_log2FC > 0.5 & p_val_adj < 0.05),file=paste(outdir,"step4",paste0("clusterall_adj0.05_logFC0.5genelist.xlsx"),sep="/"),sep='\t',row.names = TRUE, col.names = NA,quote = F)
    #top10=read.csv("/haplox/haprs/liyq/Project/HGC20220110002_sc_guangzhoudaxue/workfile/step4/clusterall_top10genelist.xlsx",sep = "\t",quote = "",header = 1)
    ...
    for (i in levels(pbmcall$seurat_clusters)){
      ...
      write.table(subset(pbmcall.markers,cluster==i & avg_log2FC > 0.5 & p_val_adj < 0.05),file=paste(outdir,"step4",paste0('cluster',i,"/cluster",i,"_padj0.05_logFC0.5genelist.xlsx"),sep="/"),sep='\t',row.names = TRUE, col.names = NA,quote = F)
